<?php 
//ovaj studentService je samo izmedu routes i dao, zato se poziva direktno preko dao objekta

/*Flight::route("GET /students", function(){
    Flight::json(Flight::student_dao()->get_all());
    //ova zagrada treba kod student_dao(), obavezno!!!!, ni slucajno je zaboraviti
    //1 linija koda umjesto 4/5
});
...
Flight::route("POST /student", function(){
    ...
    Flight::json(['message' => 'Student added succesfully', 'data' => Flight::student_dao()->add($request)]); //-> converts the results to the JSON form
...
Flight::route("PUT /student/@id", function($id){
    ...
    Flight::json(['message' => 'Student edit succesfully', 'data' => Flight::student_dao()->update($student, $id)]); //-> converts the results to the JSON form
